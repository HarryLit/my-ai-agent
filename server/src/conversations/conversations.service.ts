...
export class ConversationsService {
  ...
  addMessage(msg: {
    ...
    const id = uuid()
    this.getDb().run(
      `INSERT INTO messages (id, conversation_id, role, content, reasoning_content, model_name, thinking_time_ms, input_tokens, output_tokens, wait_time_ms, output_speed_tps)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, msg.conversation_id, msg.role, msg.content, msg.reasoning_content ?? '', msg.model_name ?? '', msg.thinking_time_ms ?? 0, msg.input_tokens ?? 0, msg.output_tokens ?? 0, msg.wait_time_ms ?? 0, msg.output_speed_tps ?? 0]
    )
